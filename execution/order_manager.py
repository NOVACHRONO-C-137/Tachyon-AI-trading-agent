...
def execute_buy(pair, crypto_amount, price):
    ...
    # Check if we have enough USD to make the trade
    usd_available = balance.get("USD", 0.0)
    ...
    if result:
        log(f"BUY order successfully submitted to Kraken engine.")
    return result
...
def execute_sell(pair, crypto_amount, price):
    ...
    crypto_available = balance.get(symbol, 0.0)
    ...
    if result:
        log(f"SELL order successfully submitted to Kraken engine.")
    return result
